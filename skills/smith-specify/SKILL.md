...
Log at these points:
1. **On invocation** — the feature description provided
2. **After system detection** — primary system identified, also_affects list
3. **After spec creation** — path to spec.md, summary of key requirements and acceptance criteria count
...
3. **Auto-detect primary system**:
   - Read all system spec files at `.specify/systems/system-*/spec.md`
   - Analyze the feature description against each system's scope, services, endpoints, and data models
   - Determine `primary_system` (the system most directly impacted) and `also_affects` (other systems touched)
   - Override the spec folder path: create the feature folder at `.specify/systems/<primary-system>/features/<NNN-short-name>/` instead of under `specs/`
   - Create the folder: `mkdir -p .specify/systems/<primary-system>/features/<NNN-short-name>/checklists`
...
5. Follow this execution flow:
    ...
    3. For unclear aspects (system detection from step 3 informs scope decisions):
...
5. Write the specification to the system-routed feature folder (from step 3). Include YAML frontmatter at the top of spec.md:
...
7. Report completion with branch name, spec file path, checklist results, and readiness for the next phase (`/smith-clarify` or `/smith-plan`).
