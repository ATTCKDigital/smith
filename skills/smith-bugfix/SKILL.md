...
**Marker before first append**: the workflow-gate denies markerless Bash redirection, so `cat >> "$SESSION"` appends are blocked until the active-workflow marker exists. Create the marker (Phase 1 step 2 helper) FIRST, then write the invocation entry immediately after — do not log before the marker.
...
As the VERY LAST action — after the PR merge, Workflow Cleanup (worktree removal +
...
cleanly if there is nothing to sync.
