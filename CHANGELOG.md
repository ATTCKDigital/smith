...
- **Deterministic Question-Asking Mechanism (feature 67-deterministic-questions)** — makes Smith's markdown Q&A contract (Context → Options with pros/cons → Recommended + reasoning, one question at a time) the single source of truth and stops the interactive `AskUserQuestion` popup from being used in its place. Previously the popup was forbidden nowhere — not by any skill, not by the graded rules — so it silently appeared whenever in-workflow instructions decayed or in freeform sessions.
  - **New `skills/smith-question/SKILL.md`** — the canonical contract for the presentation format, the one-at-a-time / wait pacing, the response grammar (`yes`/option-letter/`skip`/`done`/custom), and the persisted `questions.md` shape. Context-loading and user-invocable (`/smith-question`); it defines how questions are asked, it does not generate them. It explicitly forbids the `AskUserQuestion` popup.
  - **New `hooks/question-gate-guard.sh`** — a PreToolUse guard (matcher `AskUserQuestion`) inert for every other tool. Config-driven via `.smith/config.json` `question_gate.mode`: `deny` (shipped default — block the popup; the deny reason redirects the model to markdown), `warn` (allow + remind, reusing `security-guard-bash.sh`'s warn-only shape), `workflow-gated` (deny only when a `.smith/vault/active-workflows/` marker is active, reusing `workflow-gate.sh`'s marker check), or `off`. Fail-open: a missing/unreadable/unparseable config, an **absent** `question_gate` key, or an unknown mode all resolve to `off`, so installing the hook never silently changes an un-migrated project — a project gets `deny` only once (re-)seeded.
  - **`settings/smith-settings-fragment.json`** — one additive `PreToolUse` entry wiring the `AskUserQuestion` matcher to the new hook.
  - **New `question_gate` config key + three-site seeding** — `templates/config.default.json` gains `{"mode": "deny"}` (after `scheduled_audits`); `/smith` init's scaffold step and `/smith-update`'s new §5.1g both non-destructively merge the key into an existing `.smith/config.json`, mirroring `scheduled_audits`'s §5.1f precedent. Running `/smith-update` on an existing project is the deliberate path that activates `deny` there.
  - **Graded backstop (`settings/claude-md-template.md`)** — Rule 2 gains one compact binary sub-criterion requiring the Q&A contract and forbidding `AskUserQuestion` (no reweighting of the 100-point total); Rule 3 now points at `/smith-question` for the canonical `questions.md` shape rather than being a fourth copy. The critic (`hooks/grade-response.sh`) reads the rubric from `~/.claude/CLAUDE.md` dynamically, so the criterion is picked up once the template is propagated — no grading-engine change.
  - **Skill de-duplication + just-in-time invocation** — `skills/smith-new/SKILL.md` (Phase 5), `skills/smith-clarify/SKILL.md` (Interactive Answer Collection + the sequential ≤5-question loop), and `skills/smith/SKILL.md` (init interview) now invoke `smith-question` at the presentation step and reference the contract instead of restating it, keeping only their gate-specific content (questions.md persistence, the ≤5 budget and spec-encoding, the 29-question intake with `back` navigation). Behavior-preserving — the questions asked and their persistence are unchanged.
  - **`scripts/install.sh`** — the install preview's hardcoded "9 hooks" line is now computed dynamically (`hooks/*.sh` count), matching the adjacent dynamic skills count, so it stops drifting as hooks are added.
  - **Docs** — `docs/hooks.md` gains a summary-table row and a detailed `question-gate-guard.sh` section (all four modes, the global-install scope caveat, the fail-open / shipped-`deny`-vs-absent-`off` distinction, and the `/smith-update` propagation path).
  - **Tests** — `tests/hooks/test_question_gate_guard.sh` (15 cases: each mode, workflow-gated with/without a marker, absent/malformed config and absent/unknown key all fail-open, non-`AskUserQuestion` inert, malformed stdin no-crash, deny-is-global-with-a-marker), passing under both bash and zsh.

- **WordPress-Aware Index Defaults (feature #59, 59-wordpress-aware-index)** — teaches `/smith-index --init-system-paths` to recognize a WordPress-core checkout and seed its generated stub accordingly, plus a general (not WordPress-specific) stale-index garbage collector and a diagnostic warning for a common `system-paths.json` authoring mistake.
  - **New `scripts/smith-index/wp_defaults.py`** — pure stdlib module (`detect_wordpress(project_root)`: both `wp-load.php` file AND `wp-includes/` directory required; `wp_exclusion_rules()`: 17 stub rules — 2 directory prefixes + 15 exact root-file prefixes, each `"excluded"`, never a bare `"wp-"` prefix, which would also swallow `wp-content/`). No filesystem writes, no CLI entrypoint — imported only.
  - **`mode_init_system_paths()` WordPress branch** — when a WordPress-core checkout is detected, the 17 generated rules are prepended to the stub's `rules` list and `wp-admin/`/`wp-includes/` are skipped from the existing generic per-top-level-directory loop (avoiding a duplicate/conflicting prefix); a project without WordPress markers (or an installed tree missing `wp_defaults.py`, which degrades gracefully to `None`, same shape as the existing `path_resolver`/`meta_describe` optional-import pattern) produces byte-for-byte the same output as before this feature.
  - **`IndexRun.prune_stale_index()`** — new full-rebuild-only garbage collector: removes `.smith/index/files/**/*.meta` whose source file is gone or now resolves to `"excluded"` (reusing `_refresh_full_aggregations()`'s existing stale predicate, now wired to an actual deletion), and any `.smith/index/systems/<id>.md` left with zero files after the run. Runs if and only if this is a full, unfiltered, non-resumed rebuild — `--incremental`, `--check`, `--system <name>`, and `--resume` each leave stale state untouched, for four structurally distinct reasons (see `plan.md`). Never reads or writes `system-paths.json`, `context-manifest.json`, or `.schema-version`. The full-rebuild summary line gains a ` · <N> pruned` clause on every applicable run, including `· 0 pruned`.
  - **`_load_overrides()` diagnostic warning** — when `system-paths.json`'s top-level object has no `"rules"` key but does have a plausible alias (`overrides`, `override`, `mappings`, or `map`), one line is written to stderr naming both the alias found and `"rules"` as the correct key. Purely diagnostic — the function's return value is unchanged in every case, warning or not.
  - **`scripts/install.sh`** — one new `cp` line stages `wp_defaults.py` alongside `run.py`/`run.sh` in the smith-index runtime block, so a fresh install or the next `/smith-update` carries WordPress detection past the dev-tree graceful-degrade fallback.
  - **Docs** — `skills/smith-index/SKILL.md` gains a WordPress-detection paragraph under `--init-system-paths` and a new numbered prune step under the full-rebuild flow; `skills/smith/SKILL.md` gains a short cross-reference note (does not claim `/smith init` itself triggers `/smith-index`).
  - **Tests** — `tests/parsers/test_run_wordpress_detection.py`, `tests/parsers/test_run_load_overrides_warning.py` (new unit tests), `tests/e2e/test_full_index_rebuild_gc.sh` (new fixture test covering pruning and all four no-prune modes).

- **Scheduled Audit Subsets (feature #58, 58-scheduled-audits)** — adds a recurring, unattended `/smith-audit` pass: a second scheduler step, cadence-gated per project, plus the `/smith-audit` additions that make an unattended invocation safe.
  - **`scheduler/smith-scheduler.sh` new audits step** — a second, structurally-separate top-level loop over the same already-parsed registered-project list, running strictly AFTER the existing queue loop finishes for every project. New pure function `is_audit_due(now_date, last_run_date, cadence_days)` (never-run → due; `>= cadence_days` → due) gates each project's dispatch. For a due project it invokes `"$CLAUDE_BIN" --model "$CLAUDE_MODEL" --permission-mode bypassPermissions -p "/smith-audit --all --scheduled <subsets>"` — the identical dispatch contract shape as the queue step, reusing the same resolved `$CLAUDE_BIN`/`$CLAUDE_MODEL` and the same `scheduler.log`. `SMITH_AUDIT_DISPATCH_DRY_RUN=1` logs planned dispatches without invoking Claude, independent of the queue step's own `SMITH_SCHEDULER_DRY_RUN`. A dispatch failure is logged and the step continues to the next project; never mutates a `/smith-queue` queue file or `history/` entry. New `tests/scheduler/test_is_audit_due.sh` and `tests/scheduler/test_smith_scheduler_audits_step.sh` (dry-run only, never invokes a real `claude` process).
  - **`/smith-audit` `--scheduled` mode** — new "Phase 0: Scheduled Mode & Marker Bootstrap" section parses `--scheduled` plus a comma-separated subset list, refusing (before any marker creation or sub-audit dispatch) an unrecognized subset token, `feature` anywhere in the list (never permitted in scheduled mode — no non-interactive substitute for its interview phase), or a `--scheduled` invocation lacking `--all`/a resolvable system identifier — closing the one prompt branch a non-interactive `claude -p` call could otherwise hang on.
  - **Workflow-gate marker fix for BOTH invocation modes — the one non-opt-in behavior change** — `/smith-audit` previously had no active-workflow marker handling at all, relying on whatever marker happened to already be active or failing closed. It now bootstraps a `maintenance`-type marker (via the same `create-active-workflow.sh` helper `/smith-update`'s own Phase 0 uses, with a synthetic `--branch` label naming no real git ref — `/smith-audit` never creates a worktree or branch) before writing any report file, and clears it via `clear-active-workflow.sh` at every documented exit path (normal completion, a sub-audit crash, a report/drift/log-write failure). Unlike the rest of this feature, this fix applies to EVERY `/smith-audit` invocation — `--scheduled` and plain interactive alike — from the moment it ships, regardless of whether a project opts into `scheduled_audits`.
  - **Scoped report naming** — filename stem `<YYYY-MM-DD>-scheduled-<name>` (`<name>` = the validated subset list, hyphen-joined) replaces `<date>-full`/`<date>-full-spectrum` whenever `--scheduled` is present; directory placement is unchanged.
  - **`## Drift Since Last Scheduled Audit` block** — when a prior scheduled report resolves via the state file's `last_run.report_path`, the new report gets a per-category Critical/Warning/Info delta section (annotated `(new)`/`(resolved)`, with a category run in only one of the two audits disclosed as "not compared" rather than a false `0`), positioned before `## Executive Summary`. Omitted entirely — never rendered empty — on a project's first-ever scheduled run or when the state file is absent/corrupt.
  - **Rolling `audits-log.md`** — on successful `--scheduled` completion, exactly one line is appended to `.smith/vault/reports/audits-log.md` (timestamp, subsets, report path, severity totals); no gitignore-template change needed, since this path was already outside the managed `IGNORED` section.
  - **New `scheduled_audits` config key** — `templates/config.default.json` gains `{"enabled": false, "cadence_days": 7, "subsets": [...], "systems": "--all", "skip_pdf": true}`, a sibling of `quality`; `enabled: false` is the mandatory shipped default. `/smith` init's scaffold step and `/smith-update`'s new §5.1f both non-destructively seed this key into an already-existing `.smith/config.json`, mirroring `quality`'s own §5.1e seeding precedent.
  - **State file** — `.smith/vault/.scheduled-audits-state.json` (`last_run.date`/`.timestamp`/`.subsets`/`.report_path`/`.severity_totals`) is written by `/smith-audit` only, write-after-success — never before dispatch, never on failure. Added to the managed `.gitignore-smith-additions` template's `IGNORED` section (per-machine mutable state, not a shared artifact).
  - **Docs** — `docs/scheduler.md` gains a new "Audits Step" section (what it scans, the dispatch string, ordering, dry-run variable, Linux manual-cron note); `docs/security-model.md` gains a new "Audits step security posture" paragraph under "Scheduler Security" and, in the same edit, corrects a stale `` `"mode": "autonomous"` `` bullet to `` `complexity: autonomous` `` — the field the scheduler's code actually reads.

- **Quality Metrics Pack for `smith-build` & `smith-bugfix` (feature #57, 57-quality-metrics-pack)** — generalizes the two pipelines' hardcoded test/lint command surfaces into config-driven steps and adds three new advisory checks, all flag-only with zero terminate path.
  - **`smith-build` §3.1 "Unit Tests" → "Testing" generalized + NEW §3.1b Lint / §3.1c Typecheck** — §3.1 now reads `.smith/config.json`'s `quality.test` array and runs each command independently via `python3 subprocess.run(cmd, shell=True, timeout=quality.timeout_seconds)` (feature 56's Sub-layer D `_run_tool` mechanism, reused verbatim — no shell `timeout`/`gtimeout` wrapper); when `quality.test` is absent, empty, or `.smith/config.json` itself is absent/malformed, the CURRENT two hardcoded bullets (`pnpm test` / `poetry run pytest`) run verbatim, unchanged — zero behavior change for every project that hasn't opted in. Two new lettered sections, §3.1b "Lint" (`quality.lint`) and §3.1c "Typecheck" (`quality.typecheck`), sit between §3.1 and the existing §3.2 Playwright suite; both skip entirely (no run, no PR mention) when their array is absent or empty — neither has a legacy fallback. §3.3 "Test Failure Handling" gains one sentence naming §3.1b/§3.1c command failures and any per-command `quality.timeout_seconds` timeout as covered by the SAME existing bounded-retry/log-and-continue text (up to 3 attempts, then log and continue) — no new retry loop.
  - **NEW `smith-build` §3.4 "Coverage Check"** — runs `quality.coverage.command` exactly once (skipped entirely when absent, never retried), matches combined stdout+stderr against `quality.coverage.regex` first when configured, else a built-in three-pattern catalogue (pytest-cov `TOTAL` line, jest/istanbul `text-summary` `Lines` line, `go test -cover`), and compares the extracted percent against `quality.coverage.minimum_percent` when configured — a below-minimum result writes exactly one High finding to `/tmp/smith-build-coverage-findings.txt`; a configured-but-no-minimum percent is informational only; unparseable output (or a command timeout) is disclosed as "coverage output unparsed," never a build failure.
  - **NEW `smith-build` §5.3.2 "Pre-PR Function-Length Scan"** — reuses `/tmp/smith-build-changed.txt` (§5.3), the same extension filter/exclude list §5.3/§5.3.1 already use, and the same installed-path-preferred parser resolution, then imports `scripts/parsers/meta_describe.py` (it has no CLI entrypoint, so it cannot run as a subprocess) via `sys.path.insert(0, dir); import meta_describe as md` — the exact pattern `describe_write.py`/`describe_discover.py` already use in production — and calls its existing `qualifying_methods(parsed, threshold=quality.function_length.soft)` completely unchanged. Buckets each result by `body_lines`: `>= quality.function_length.decompose` (default 100) is listed individually in `/tmp/smith-build-function-length-findings.txt`; `>= soft` (default 50) and `< decompose` is folded into exactly one trailing `+ N soft-tier warnings` line, omitted when N=0.
  - **§5.4 PR template gains two conditional sections** — "Function Length Warnings" (from §5.3.2's findings file) then "Quality Metrics" (from §3.4's findings file), both appended after "Supply-Chain Review" and before "Release notes," each omitted entirely when its source file is empty/absent. Both are explicit flags, never blockers — the PR always opens.
  - **`smith-bugfix` §5.3 "Lint" generalized the same way** — the ONLY `smith-bugfix` change this feature makes: reads `quality.lint`, same per-command timeout-bounded `subprocess.run` mechanism, verbatim hardcoded fallback (`pnpm lint` / `poetry run ruff check .`) when absent/empty/malformed. `smith-bugfix` gains no coverage check, no function-length scan, and no new PR-body section from this feature.
  - **New `quality` config key + three-site non-destructive seeding** — `templates/config.default.json` gains a `quality` key (`test`/`lint`/`typecheck` arrays, `coverage: {command, minimum_percent, regex}`, `function_length: {soft: 50, decompose: 100}`, `timeout_seconds: 120`, `excludes: []`), a sibling of `security_review`/`supply_chain`, never nested under either — no `enforcement_tier`-shaped field anywhere in this schema, since this feature has no terminate path to gate. `/smith` init's scaffold step and `/smith-update`'s new §5.1e both non-destructively merge this key into an already-existing `.smith/config.json`, mirroring `supply_chain`'s own §5.1d seeding precedent exactly.
  - **Flag-only, no-terminate posture throughout** — consistent with features 54/55/56's own flag-only precedent for their non-secret findings, no finding this feature produces, from §3.1b/§3.1c/§3.4/§5.3.2, at any severity, blocks, delays, or terminates a build or bugfix.
  - **Tests** — `tests/skills/test_smith_build_function_length_scan.sh` (new, 7 cases: decompose-tier function listed individually, soft-tier functions folded into one line, soft-tier never listed individually, sub-threshold function absent entirely, excluded-path file produces no findings even with an oversized function inside, and a `quality.function_length` override correctly shifting tier assignment) and a new regression case in `tests/hooks/test_config_default_seed.sh` confirming a fresh project's seeded `.smith/config.json` now also carries `quality`.

- **Supply-Chain & License Review Pass for `smith-build` (feature #56, 56-supply-chain-gate)** — adds a whole-project dependency CVE scan and license inventory to the build pipeline, distinct from the diff-scoped Code-Content Security Review Pass (feature #55) that runs immediately before it.
  - **New `smith-build` `## Phase 3.7: Supply-Chain Review Pass`** — runs exactly once per build, strictly after Phase 3.6 completes and before Phase 4, scanning the full project (`WORKTREE_PATH`, never `BASE_BRANCH` — this is the pipeline's only whole-project, non-diff-scoped pass). Sub-layer D (`scripts/security/dependency-scan.py`/`.sh`) discovers every `package.json`/`pyproject.toml`/`requirements.txt`/`go.mod`/`Cargo.toml` manifest via a new shared `_manifest_discovery.py` module, prefers a single whole-repo `osv-scanner`/`trivy` pass when either is present, and otherwise falls back per manifest to `npm audit` (lockfile-gated) or `pip-audit` (poetry/pip); `grype` is detected and disclosed but never invoked in this v1. Sub-layer L (`scripts/security/license-inventory.py`/`.sh`) walks `node_modules/` and each poetry manifest's environment (`poetry run python3` + `importlib.metadata`, never system Python) to build a repo-wide license inventory and flag any package matching a configured `supply_chain.license_policy.deny` entry. Both scripts are pure stdout emitters; the SKILL.md-level phase is the sole writer of `/tmp/smith-build-supply-chain-findings.txt` and `/tmp/smith-build-supply-chain-scan-status.txt`, merging both sub-layers' findings/status lines and summing their low-severity counts into one trailing line.
  - **Flag-only, no terminate path — contrasted explicitly with Phase 3.6** — unlike the Code-Content Security Review Pass, this phase has no decision table, no `enforcement_tier`-shaped config field, and no terminate branch of any kind: not even a Critical CVE with a known exploit, not even a deny-listed license on a production package, can block, delay, or terminate the build. Phase 4 always begins next, unconditionally. Zero `Write`/`Edit` calls to the working tree for any finding, from either sub-layer, ever.
  - **New `supply_chain` config key + non-destructive seeding** — `templates/config.default.json` gains a `supply_chain` key (`cve_scan: true`, `license_inventory: true`, `license_policy: {allow: [], deny: []}`, `timeout_seconds: 60`, `excludes: []`), a sibling of `security_review`, never nested under it. `/smith` init and `/smith-update` (new §5.1d) both non-destructively merge this key into an already-existing `.smith/config.json`, mirroring `security_review`'s own seeding precedent exactly.
  - **`smith-audit` Dependencies sub-audit upgrade** — item 7 now names the concrete mechanism backing it: the same `detect-scanners.sh`/`dependency-scan.py`/`license-inventory.py` scripts Phase 3.7 uses, run whole-project, MCP-independent, with no separate or duplicated implementation — closing feature #55's own NFR-4 intent, which explicitly named this future consumer.
  - **`detect-scanners.sh` extension** — the presence-detection loop now also checks `osv-scanner`, `grype`, `trivy`, `pip-audit`, `licensee`, and `syft` (nine tools total), same `<tool>=present|absent` contract; `scripts/install.sh`'s security-copy stanza gains five new per-file `cp` lines for `_manifest_discovery.py`, `dependency-scan.{py,sh}`, and `license-inventory.{py,sh}` so `/smith-update` actually refreshes them on already-installed projects.
  - **Docs** — `docs/security-model.md` gains a new "Supply-Chain & License Review" section (two sub-layers, an explicit no-redaction note since findings carry no secret-shaped content, the no-terminate contract, honest disclosure, config home) and extends "What to Audit Before Enabling" from seven files to nine.

- **Inline Security Review Pass for `smith-build` (feature #55, 55-security-review-pass)** — adds a diff-content security review layer to the build pipeline, distinct from the existing `PreToolUse` guard hooks (which inspect tool-call intent, never code content).
  - **New `smith-build` `## Phase 3.6: Security Review Pass`** — three layers evaluated against the full branch diff vs `$BASE_BRANCH`, inserted after Phase 3.5 (Clean Code Review) and before Phase 4: Layer 1 built-in secret scan (`scripts/security/secret-scan.sh` + `secret_scan.py`, always runs, merges `gitleaks` findings additively when present), Layer 2 SAST (`semgrep`/`bandit`, presence-detected via `scripts/security/detect-scanners.sh`, silently skipped when neither is installed — never added as an installed dependency), and Layer 3 LLM review (a single subagent evaluated against a fixed 10-area rubric — injection, auth/authz, secrets handling, unsafe deserialization, path traversal, SSRF, crypto misuse, sensitive-data logging, dependency-adjacent code smells (non-CVE), race conditions/TOCTOU). Every finding carries severity/`path:line`/category/rationale/layer and is evaluated against a tier × severity × layer decision table; this phase never auto-fixes anything, for any finding, from any layer.
  - **Unconditional, non-bypassable secret terminate** — any Layer 1 Critical secret finding always terminates the build before Phase 4, regardless of the configured `enforcement_tier`, with no runtime-confirmation escape hatch — the `# smith-secret-scan: allow` per-line marker is the sole false-positive remedy, applied before the scan runs. This is Smith's second non-bypassable denial, after the browser-production confirm-gate (feature #53); `docs/security-model.md`'s "sole non-bypassable" framing is now scoped explicitly to the three `PreToolUse` guard hooks and cross-references this new one.
  - **`opus`-pinned Layer 3, `review_model` override** — the LLM review subagent defaults to `opus`; the optional `security_review.review_model` key (`haiku`/`sonnet`/`opus`/`fable`) overrides the model per project.
  - **`smith-bugfix` gains a Layer-1-only gate** — a secret scan now runs immediately before `### 7.1 Commit`, with the same unconditional terminate semantics as `smith-build`; Layer 2 (SAST) and Layer 3 (LLM review) never run in `smith-bugfix`, keeping its lightweight pipeline otherwise unchanged for a clean scan.
  - **`scripts/install.sh` install stanza** — stages `scripts/security/{secret-scan.sh,secret_scan.py,detect-scanners.sh}` to `~/.smith/scripts/security/`, mirroring the existing `smith-index`-family copy precedent; without it these scripts would only ever be reachable in this repo's own dev tree, never on an installed project.
  - **New `security_review` config key + non-destructive seeding** — `templates/config.default.json` gains a `security_review` key (`enforcement_tier: "flag"`, `review_model: "opus"`, per-layer `layers` toggles, `excludes`, `allowlist_globs`), distinct from the pre-existing orphaned `security` block (BANK-027: no guard hook reads it) and from `.smith/security-config.json`. `/smith` init and `/smith-update` (new §5.1c) both non-destructively merge this key into an already-existing `.smith/config.json`, closing the gap where the whole-file template seed only ever helps brand-new projects.
  - **Redaction guarantee** — every finding's excerpt is masked (first-4/last-4 characters, or a fixed literal for PEM/GCP-JSON matches); the scanner never prints a secret's actual value, whether the finding is flagged in a PR body or recorded in a hard-stop's vault session log entry.
  - **Docs** — `docs/security-model.md` gains a new "Code-Content Security Review" section (three layers, the terminate rule, honest presence-disclosure, config home) and extends "What to Audit Before Enabling" from four files to seven.

- **Clean Code Review Pass for generated code (feature #54, 54-clean-code-review-pass)** — closes the gap where PR #49's clean-code integration was generation-time-only advice with nothing re-checking the finished diff before it shipped.
  - **New `smith-build` `## Phase 3.5: Clean Code Review Pass`** — a single Sonnet-run subagent (not Haiku — this pass makes auto-fix judgment calls, not narrow classification), inserted between Phase 3 (Testing) and Phase 4 (Spec Updates), evaluates the full branch diff vs `$BASE_BRANCH`. It reads `skills/smith-clean-code/SKILL.md`'s Review Process/Decision Rules/What You Should Avoid sections by HEADING (worktree copy first, `~/.claude/skills/` install fallback), never by line number, so the rubric can drift without breaking the reference. Auto-fixes only safe, unambiguous, behavior-preserving findings — a Critical finding needing a behavior change is never auto-fixed, always flagged. A strictly bounded, non-repeating cycle: one review, at most one fix-application batch, at most one full Phase 3 re-run (only if `fixes_applied > 0`) — no re-review, no second fix batch, ever. `.meta` coverage for auto-fix edits relies entirely on the existing layered mechanisms (smith-new's per-edit instruction, or the passive §5.3.1 scan for standalone builds) — no new `.meta`-write machinery was added. Remaining findings are written by the review subagent to `/tmp/smith-build-clean-code-findings.txt` (Medium/High/Critical listed individually, Low findings folded into one `+ N low-severity notes` line) and surfaced in a new flag-never-block `## Clean Code Review` PR-body section — the PR always opens regardless of findings.
  - **`smith-bugfix`/`smith-implement` checklist parity** — both files' implementation-rules text now spells out the same inline six-bullet clean-code checklist `smith-build` Phase 2 already showed implementers (small single-responsibility units, intention-revealing names, guard clauses, separation of concerns, no dead code/duplication, reuse over recreation), closing the "Phase 2 has the checklist, everyone else has a pointer" gap — replacing `smith-bugfix`'s constitution-pointer-only sentence and `smith-implement`'s single-line Clean Architecture Policy bullet. `smith-bugfix` gets checklist parity only, no automated review pass, and its existing "Do NOT: refactor surrounding code / add features beyond the fix / modify unrelated files / add unnecessary abstractions" constraints are unchanged.

- **Playwright MCP browser-interaction safety guard + MCP-first verification convention (feature #53, 53-mcp-browser-access)** — closes the exploration-flagged parity gap where `mcp__playwright__*` browser tools had no `PreToolUse` guard alongside the existing `Bash` and `Write|Edit|NotebookEdit` guards, and gives agents a config-gated path to drive the operator's own already-logged-in browser (Playwright MCP bridge/extension mode) instead of a throwaway unauthenticated headless session, with a hard fallback whenever that isn't safe or available.
  - **Guard hook + tests** — new `hooks/security-guard-mcp-browser.sh` (`PreToolUse`, `mcp__playwright__`) always allows read-only tools (navigate/snapshot/screenshot/console/network/wait_for/tabs) on any target, including production. Interaction tools (click/type/fill_form/select_option/press_key/drag/hover, and `browser_evaluate` — always interaction-class, no JS-source heuristic) are gated: denied outright when `browser_verification.allow_interactions` is `false` (master kill-switch, downgradable by `warn_only_mode`), and denied against a production-labeled or unclassified target (fail-safe default; `localhost`/`127.0.0.1`/`::1` are always staging) unless a matching `.smith/vault/.mcp-browser-confirmed` record exists for the current `.smith/vault/.mcp-browser-target` — that production confirm-gate is the one denial `warn_only_mode` can never downgrade or bypass. No-ops silently when the vault or `.smith/security-config.json` is absent. `tests/hooks/test_security_guard_mcp_browser.sh` covers all 12 required cases (read-only-always-allow, staging/localhost allow, production deny with/without confirmation, stale-confirmation invalidation, non-bypassable production gate under `warn_only_mode`, kill-switch downgradability contrast, `browser_evaluate` gated identically to `browser_click`, absent/malformed config no-op) and passes byte-identical under both bash and zsh.
  - **Registration + seeding** — registered in `settings/smith-settings-fragment.json`'s `PreToolUse` array (matcher `mcp__playwright__`). `/smith` init and `/smith-update` now seed `browser_verification.urls` (`{"staging": [], "production": []}`) into `.smith/security-config.json` non-destructively, so every project has a well-formed schema for the guard to read from day one. `browser_verification.mcp_mode` stays in `.smith/config.json` (agent-read only — the guard itself never reads it).
  - **Convention template** — new `## MCP-First Browser Verification` section in `templates/claude-md-additions.md`: tool-presence detection, the `mcp_mode` (`extension`/`sandbox`/`off`, never inferred from tool presence) config key, the silent-skip fallback idiom, the non-interactive/scheduler hard-skip rule, the staging/production URL table schema, and the full decision chain plus its fast-fail-no-retry rule — the single canonical place every consuming surface references rather than restating. `/smith-index --migrate-templates`'s detection list is extended in both places that matter: the doc-facing bullet list in `skills/smith-index/SKILL.md`, and the actual code-enforced `CLAUDE_SECTIONS` constant in `scripts/smith-index/run.py` (found unpatched during verification — the doc list alone would not have made the real runner detect/append the new section).
  - **Agent & skill prose** — `skills/smith/agents/product-manager.md`, `senior-qa.md`, `staff-frontend.md`, `staff-fullstack.md`, and `skills/smith-audit/SKILL.md`'s UX/SEO sub-audits all reference the MCP-first-with-fallback contract ahead of their Playwright-driven verification steps; `skills/smith/SKILL.md`'s Q19 CLAUDE.md-generation block now points at the convention section by name instead of duplicating it.
  - **Docs** — `docs/security-model.md` ("two guards" → "three", new per-guard subsection, updated hook-event table, 4th audit item) and `docs/hooks.md` (new Hook Summary row + Detailed Reference subsection, noting the two `.smith/vault/` state files it writes, unlike its inspection-only siblings) document the new guard alongside its siblings.

- **Two utility skills bundled with the plugin (`clean-code`, `to-mermaid`)** — vendored `skills/clean-code/SKILL.md` (framework-agnostic Clean Code / Clean Architecture refactoring assistant, Uncle Bob principles) and `skills/to-mermaid/SKILL.md` (converts plans, workflows, and system flows into Mermaid diagrams, auto-selecting the diagram type). Both are auto-discovered via `plugin.json`'s `"skills": "skills/"` glob. Unlike every other bundled skill these are **not** `/smith-*` workflow commands but general-purpose developer utilities, listed under a new "Utilities" row in the README skills table (real total corrected to 33 — the header had drifted).
  - **Installer delivery (both install paths).** `scripts/install.sh` copied only `skills/smith` + `skills/smith-*`, so the `curl | bash` / `/smith-update` path would have silently skipped these two while the plugin-install path picked them up. The copy loop now globs `skills/*` (every shipped skill, smith-namespaced or not, future-proof), and the "Copy N skills" preview is computed dynamically instead of a hard-coded (and stale) `27`. `scripts/uninstall.sh` stays symmetric — it removes the bundled non-smith skills **by explicit name** (`clean-code`, `to-mermaid`), never a blanket `skills/*` glob, so a user's own global skills are never deleted. `tests/install.smoke.sh` now asserts both skills install and are removed on uninstall.

- **Clean-code guidance made subagent-safe + reuse lookup made cheap** — the `/smith-build` implementation rule that referenced the `/clean-code` skill now inlines a compact clean-code checklist directly, since build subagents may not have skill-loading access (a bare reference would have been inert). `/smith-new`'s reuse-before-create step gains a note to build the `.smith/index/` manifest once up front so `/smith-navigate` reuse detection is fast and comprehensive rather than falling back to grep.

- **Deterministic response stamp hook (`stamp-response.sh`)** — new `hooks/stamp-response.sh` (`Stop`) appends the `YYYY-MM-DD HH:MM:SS — <branch-name>` stamp (Rule 6) to each main-session turn via a string-level append, replacing the fail-open critic enforcement. Previously the stamp was enforced only by `grade-response.sh`'s Haiku critic against Rule 6; on a critic timeout, JSON parse failure, or retry-budget exhaustion the hook defaulted to pass and let a turn through with no stamp — so the stamp appeared reliably on heavily-scaffolded projects but silently dropped on minimal ones.
  - **Mechanism** — resolves the branch from the Stop payload's `cwd` (falls back to a bare timestamp outside a git repo, matching Rule 6's wording), prints the stamp to stdout for Claude Code to surface as appended turn context. Idempotent: a no-op when the transcript's last assistant text already ends with a well-formed stamp, and short-circuits on the `stop_hook_active` re-fire. Always exits 0 — never blocks the stop.
  - **Rule 6 change** — the datetime-stamp sub-criterion was removed from `settings/claude-md-template.md` Rule 6 (the `python3` sub-criterion and the rule's weight are unchanged); a note documents that the stamp is now hook-enforced and must not be re-added to the critic rubric.
  - **Wiring** — registered as its own `Stop` entry in `settings/smith-settings-fragment.json` (before `grade-response.sh`). Distributed via the standard install/`/smith-update` path: `install.sh` already globs `hooks/*.sh` and merges the fragment with a `(matcher + hooks-array)` dedupe, and `dedupe-settings.sh` uses the same key — so the new entry is carried exactly once and is idempotent across re-runs. No bespoke install machinery; no change to `install.sh` or `dedupe-settings.sh` required.

- **User prompt logging in session logs (42-prompt-logger)** — new `hooks/user-prompt-logger.sh` (`UserPromptSubmit`) appends each user's prompt — **verbatim, full text**, with an `HH:MM:SS` timestamp — to the current `.smith/vault/sessions/<session>.md` as a `### [HH:MM:SS] User prompt` blockquote block, interleaved chronologically with `metrics-tracker.sh`'s tool-call lines. Until now, user prompts lived only in Claude Code's per-user global JSONL transcripts (`~/.claude/projects/<slug>/*.jsonl`), which are never team-shared; the session log records only tool calls. This hook surfaces prompts in the `/smith-sync`-shared session log so team leads can see what teammates asked the agent to do.
  - **Wiring** — added as a second entry under the existing `UserPromptSubmit` array in `settings/smith-settings-fragment.json`, after `context-loader.sh` (Q2 answer A — context injection is never delayed by the logger). Distributed via the standard `/smith-update` path (Q4 answer A); no bespoke install machinery.
  - **Verbatim parsing** — the hook parses the `prompt` field with `python3 json.loads` (not the sed-based extraction `context-loader.sh` uses), so multi-line prompts, embedded quotes, and Unicode survive intact. Blank lines become a bare `>` so the blockquote stays contiguous.
  - **Scope** — main-session prompts only; Claude Code does not fire `UserPromptSubmit` for sub-agents, so no filtering is needed. Slash-command prompts ARE logged (Q3 answer A) — they show which workflows people launched and the args passed.
  - **Privacy (intentional)** — prompts are stored verbatim including any pasted secrets, an accepted trade-off for internal team repos where session logs are shared. Documented in the hook header and `docs/hooks.md`; not gitignored (session logs are already team-shared via the #37/#40/#41 selective git policy).
  - **Tests** — `tests/hooks/test_user_prompt_logger.sh` (verbatim multi-line capture, quotes/unicode, no-vault no-op, empty-prompt skip, JSON validity of the settings fragment, workflow-summary aggregation unaffected by prompt blocks).
...
- **Clean-architecture guidance baked into every code-generating workflow** — new/changed code is now steered toward small, single-responsibility files and component reuse at generation time, instead of only being flagged after the fact. A new **Clean Architecture Policy** in `templates/constitution-additions.md` (alongside the File Size Policy) is the canonical, inherited source: single responsibility, reuse over duplication (via `/smith-navigate` / `.smith/index/`), intention-revealing names, small files, with a scope note that `/smith-bugfix` applies it only to code the fix adds/changes (no refactoring untouched code). The code-writing skills reference it:
  ...
  - `skills/smith-implement/SKILL.md` and `skills/smith-bugfix/SKILL.md` — implementation rules now reference the Clean Architecture Policy (bugfix scoped to added/changed code only, preserving its minimal-change contract).

  All guidance is **advisory** — consistent with Smith's flag-never-block convention. Decisions recorded in `specs/questions/smith-new-clean-code.md`.

- **Skill-count strings reconciled to 33** — the skill count had drifted across the repo (README badge said 29, intro prose 28, installer section 26; `docs/getting-started.md` 25; `docs/architecture.md` 25; the skills-table header 29). All now read **33** — the real total (31 `smith-*` skills + `clean-code` + `to-mermaid`). The installer's runtime preview is computed dynamically (see above) so this class of drift can't recur there.
...
- **`hooks/active-workflow-janitor.sh`** — Stop-hook sweep for stale `.smith/vault/active-workflows/*.yaml` markers. Removes files whose branch is gone (locally and on origin) or already merged into `origin/main`, **AND the marker is past a 1-hour grace period** (`SMITH_JANITOR_GRACE_SECONDS` overrides). The grace period prevents false-sweeping freshly-created markers where `branch tip == main tip` transiently (smith-new creates the marker BEFORE the worktree and any commits, so the naive is-ancestor check would have matched). Project root is resolved via `git rev-parse --git-common-dir` so subagent contexts (where `CLAUDE_PROJECT_DIR` is unset and `$PWD` is the worktree, not the primary repo) still find the marker correctly — same fix shape that landed in PR #20's workflow-gate. Belt-and-suspenders with the existing `clear-active-workflow.sh` helper: skill-level cleanup still handles the happy path, and the hook catches orphans from crashed sessions, interrupted workflows, or skills that slipped through. Runs with hook-level permissions, so it works under a `Bash(rm:*)` deny rule without an allow-list entry. Registered in `settings/smith-settings-fragment.json` so `scripts/install.sh` wires it into `~/.claude/settings.json` on install.
